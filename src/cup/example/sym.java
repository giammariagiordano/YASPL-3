
// ----------------------------------------------------
// The following code was generated by CUP v0.11b 20141204 (SVN rev 60)
// ----------------------------------------------------

package cup.example;

/** CUP generated interface containing symbol constants. */
public interface sym {
  /* terminals */
  public static final int CHAR_CONST = 47;
  public static final int LGPAR = 23;
  public static final int CHAR = 8;
  public static final int GE = 35;
  public static final int UMINUS = 36;
  public static final int DOUBLE_CONST = 45;
  public static final int DOUBLE = 6;
  public static final int SEMI = 21;
  public static final int INT = 4;
  public static final int OUT = 19;
  public static final int FOR = 17;
  public static final int MINUS = 27;
  public static final int WRITE = 41;
  public static final int INOUT = 20;
  public static final int AND = 38;
  public static final int NOT = 37;
  public static final int LT = 30;
  public static final int RPAR = 22;
  public static final int OR = 39;
  public static final int IN = 18;
  public static final int BOOL = 5;
  public static final int RGPAR = 24;
  public static final int COMMA = 25;
  public static final int DIV = 29;
  public static final int PLUS = 26;
  public static final int ASSIGN = 32;
  public static final int IF = 12;
  public static final int ID = 48;
  public static final int LE = 34;
  public static final int EOF = 0;
  public static final int INT_CONST = 44;
  public static final int TRUE = 10;
  public static final int error = 1;
  public static final int MOD = 42;
  public static final int START = 3;
  public static final int EQ = 33;
  public static final int HEAD = 2;
  public static final int TIMES = 28;
  public static final int ELSE = 14;
  public static final int READ = 40;
  public static final int WHILE = 15;
  public static final int THEN = 13;
  public static final int LPAR = 43;
  public static final int STRING = 7;
  public static final int DEF = 9;
  public static final int FALSE = 11;
  public static final int STRING_CONST = 46;
  public static final int GT = 31;
  public static final int DO = 16;
  public static final String[] terminalNames = new String[] {"EOF", "error", "HEAD", "START", "INT",
      "BOOL", "DOUBLE", "STRING", "CHAR", "DEF", "TRUE", "FALSE", "IF", "THEN", "ELSE", "WHILE",
      "DO", "FOR", "IN", "OUT", "INOUT", "SEMI", "RPAR", "LGPAR", "RGPAR", "COMMA", "PLUS", "MINUS",
      "TIMES", "DIV", "LT", "GT", "ASSIGN", "EQ", "LE", "GE", "UMINUS", "NOT", "AND", "OR", "READ",
      "WRITE", "MOD", "LPAR", "INT_CONST", "DOUBLE_CONST", "STRING_CONST", "CHAR_CONST", "ID"};
}

